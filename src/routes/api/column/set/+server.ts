...
export const PATCH: RequestHandler = async ({ locals, url }) => {
	...
	const widgetsArrayUrlParam = url.searchParams.get('widgets');

	if (!column)
		return json({ success: false, error: 'No column was specified' }, { status: 400 });
	if (!widgetsArrayUrlParam)
	...
	database.layouts.setColumnWidgets(locals.layout.id, column, JSON.parse(widgetsArrayUrlParam));
